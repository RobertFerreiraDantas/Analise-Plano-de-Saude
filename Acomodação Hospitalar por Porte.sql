...
SELECT
	"PORTE_OPERADORA","ACOMODACAO_HOSPITALAR",COUNT("ACOMODACAO_HOSPITALAR") AS quantidadeS
FROM
	tabela
WHERE
	"ACOMODACAO_HOSPITALAR" NOT IN ('Não se Aplica','Não Informado')
	AND "PORTE_OPERADORA" != 'Sem beneficiários'
GROUP BY
	"PORTE_OPERADORA","ACOMODACAO_HOSPITALAR"